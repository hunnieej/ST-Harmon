...
class Harmon(nn.Module):
    ...
    # NOTE : Original sampling function for Harmon
    @torch.no_grad()
    def sample(self,
        ...
        # prompt만 먼저 LLM에 넣어서 cache 생성 : past_key_values > 이후 step 마다 image token을 이어 붙일 때 cache 재사용
        ...
        for step in indices:
            ...
            # cfg schedule follow Muse : 처음에는 guidance 약하게, 점점 강하게 or 상수
