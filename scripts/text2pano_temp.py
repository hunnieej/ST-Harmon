...
from scripts.cylindrical_functions_temp import build_projection_ssot_maps
from scripts.importance_functions_temp import (compute_global_cfg_importance_map,
                                          build_perm_by_phase,
                                          make_global_perm_pool_diverse_anchors,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--anchor_pool_mult", type=int, default=4)
    parser.add_argument("--anchor_alpha", type=float, default=0.7)
    parser.add_argument("--anchor_dist", type=str, default="manhattan", choices=["manhattan","euclidean"])
    parser.add_argument("--anchor_warmup_ratio", type=float, default=0.15)  # 초기 15%는 spatial stratified
    parser.add_argument("--anchor_topk", type=int, default=32)
    parser.add_argument("--anchor_neighbor_mode", type=int, default=4, choices=[4,8], help="4 or 8 neighbor mode for anchor expansion.")
    parser.add_argument("--projection_type", type=str, default="cyl_yaw",choices=["cyl_yaw", "sph_demo"], help="projection backend for precomputing SSOT maps.")
    ...
    #     device=device,
    # )
    maps = build_projection_ssot_maps(
        projection_type=getattr(args, "projection_type", "cyl_yaw"),  # argparse 추가 안 했으면 기본값 사용
        m=m,
        n=n,
        tile_m=tile_m,
        tile_n=tile_n,
    ...
    read_lin_list = maps.read_lin_list
    writer_lin_list = maps.writer_lin_list
    writer_pix_list = maps.writer_pix_list

    print(f"[OK] Precomputed SSOT maps: type={maps.projection_type}", flush=True)
    ...
    if args.importance=="True":
        ...
        # global_perm= build_perm_by_phase(
        #     step=0,
        ...
        # )
        warmup_steps = int(args.num_iter * args.anchor_warmup_ratio)
        global_perm = make_global_perm_pool_diverse_anchors(
            score_map=importance_map,
            global_mask=global_mask,
            prev_perm=None,
            topk=args.anchor_topk,
            pool_mult=args.anchor_pool_mult,
            alpha=args.anchor_alpha,
            dist_mode=args.anchor_dist,
            noise_std=0.01,
            step=0,
    ...
    update_steps = set()
    if args.importance=="True":
        s1 = int(args.num_iter * 0.25)
        s2 = int(args.num_iter * 0.5)
        if 0 <= s1 < args.num_iter:
            update_steps.add(s1)
        if 0 <= s2 < args.num_iter:
            update_steps.add(s2)
    ...
    for step in tqdm(range(args.num_iter), desc="Global-step", disable=False):
        ...
        upd_int_flat = torch.zeros(bsz, L, device=device, dtype=model.dtype)
         ...
         # ---- optional: update permutation (re-order unknown only) ----
        if args.importance == "True" and step in update_steps: 
            importance_map = compute_global_cfg_importance_map(
                model=model,
                step=step,
                num_iter=args.num_iter,
            ...
            if args.save_steps == "True":
                save_importance_heatmap(importance_map, out_dir / f"importance_heatmap_step{step:02d}.png")
                save_rank_map(global_perm, m, n, out_dir / f"rank_map_step{step:02d}.png")
